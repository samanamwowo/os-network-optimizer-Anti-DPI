...
apply_dns_optimization() {
    ...
    # If resolv.conf is a symlink managed by systemd-resolved/NetworkManager,
    # replace it with a plain static file so our settings actually stick.
...
# ============================================================
#  1c) IRQ balancing across CPU cores
# ============================================================
apply_irq_balancing() {
    log_step "Enabling IRQ balancing across CPU cores..."
    ...
    if systemctl is-active irqbalance >/dev/null 2>&1; then
        log_ok "irqbalance is installed, enabled, and running"
    else
...
run_uninstall() {
    ...
    if [[ -f /etc/resolv.conf.antidpi.bak ]]; then
        ...
        log_info "No backup found, leaving current /etc/resolv.conf as-is."
    fi
